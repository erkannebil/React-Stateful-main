...
export default function Programcilar() {
  ...
  const [programcilar, setProgramcilar] = useState([
  ...
  let [winnerProgrammer, setWinnerProgrammer] = useState('');
  ...
  const oneCikaninIsmi = () => {
	...
	// bu bölgesinden her iki state dilimini de "görebilmemiz"dir.
       return winnerProgrammer ? winnerProgrammer.isim : '';
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: winnerProgrammer ? 'gold' : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => setWinnerProgrammer(dev)}>Kutla</button>
            </div>
          )
        }
      </div>
